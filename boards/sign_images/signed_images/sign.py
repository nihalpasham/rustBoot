...
if encrypt:
    if '.' in image_file:
        ...
        encrypted_output_image_file += "_v" + \
            str(fw_version) + "_signed_and_encrypted.bin"
    else:
        encrypted_output_image_file = image_file + "_v" + \
            str(fw_version) + "_signed_and_encrypted.bin"
...
if not sha_only and not manual_sign:
    ...
    if sign == 'ecc256':
        ...
        ecc.decode_key_raw(
            wolfboot_key_buffer[0:31], wolfboot_key_buffer[32:63], wolfboot_key_buffer[64:])
...
if not manual_sign:
    ...
    if len(buf) != HDR_SIGNATURE_LEN:
        print("Wrong signature file size %d, expected %d" %
              (len(buf), HDR_SIGNATURE_LEN))
        sys.exit(4)
    signature = buf
